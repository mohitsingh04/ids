...
export default function CallerList() {
  ...
  const { authUser, authLoading, getRoleById, organization } =
    useOutletContext<DashboardOutletContextProps>();

  const [loading, setLoading] = useState(true);

  const getAllCallers = useCallback(async () => {
    setLoading(true);
    try {
      const response = await API.get(`/user/organization/${organization?._id}`);
      const data = response.data;
      const finalData = data?.map((item: UserProps) => {
        return { ...item, role: getRoleById(item.role) };
      });
      setUsers(
        finalData?.filter((item: UserProps) =>
          !["super admin", "admin"]?.includes(item?.role)
        )
      );
    } catch (error) {
      ...
      },
      {
        ...
        value: (row: UserProps) => (
          <div className="flex space-x-2">
            {!authLoading && (
              <>
                {matchPermissions(authUser?.permissions, "Read Caller") && (
                  <TableButton
                    Icon={LuEye}
                    color="blue"
                    size="sm"
                    buttontype="link"
                    href={`/dashboard/caller/${row._id}`}
                  />
                )}
                {matchPermissions(authUser?.permissions, "Update Caller") && (
                  <TableButton
                    Icon={GiPencil}
                    color="green"
                    size="sm"
                    buttontype="link"
                    href={`/dashboard/caller/${row._id}/update`}
                  />
                )}
              </>
            )}
